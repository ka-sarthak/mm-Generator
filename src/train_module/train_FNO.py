...
exp_name = f"nonGAN_FNO_256_nontriv_from_output"

## training parameters
continued = False 	# ArgumentCheck(sys.argv[1:])
train_val_test_split = [800,100,100]
epoch_goal 		= 200
learning_rate	= 0.001
batch_size 		= 8 # vary (1,2,4,8,16,32,64)
gamma			= 0.5
step_size		= 100
...
num_heads = 5
modes 	= 20
width 	= 32 # vary (8,16,32,64)

## make paths and directories
main_path         = "../"
path_data         = os.path.join(main_path,  "../../article1_FNO_UNet/data/elasto_plastic/256/")
path_save_model   = os.path.join(main_path,  "saved_models/nonGAN/FNO/")
path_training_log = os.path.join(main_path, f"training_log/nonGAN/FNO/{exp_name}/")
...
if epoch_completed == 0:
	with open("train_nonGAN_FNO.py","r") as input:
		for line in input:
			logfile.write(line)
	logfile.write("\n----------------------\n")
	with open("generator_models/FNO.py","r") as input:
		for line in input:
			logfile.write(line)
	logfile.write("\n----------------------\n")
...
while(ep <= epoch_goal + epoch_completed):
	...
	print(f"{ep} \t t:{t2-t1} \t g_train_loss:{g_loss_epoch} \t g_val_loss:{g_loss_validation[-1]}", flush=True)
	ep+=1
